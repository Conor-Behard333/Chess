...
public class Run extends Application {
    ...
    private void movePiece(ImageView[][] board, ImageView clickedSquare, String colour, double newX, double newY, boolean opponent) {
        ...
        pickPiece = true;
        movePiece = false;
        int promotionCol = searchForPawnPromotion(board, colour);
        if (promotionCol >= 0) {
            promote(board, colour, promotionCol);
        } else {
    ...
    private String getChoice() {
        ...
        dialog.setContentText("Pick a piece you want to replace the pawn with");
    ...
    private int searchForPawnPromotion(ImageView[][] board, String colour) {
        for (int i = 0; i < 8; i++) {
            if (colour.equalsIgnoreCase("white")) {
                if (board[i][0].getId().contains(colour) && board[i][0].getId().contains("Pawn")) {
                    return i;
                }
            } else {
                if (board[i][7].getId().contains(colour) && board[i][7].getId().contains("Pawn")) {
                    return i;
                }
            }
        }
